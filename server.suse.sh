...
zypper -n in sipcalc net-tools-deprecated 
...
#####DIRECTORIES
myDNSDIR="/var/lib/named"
myLDAPCONFDIR="/etc/openldap"
myLDAPDATADIR="/var/lib/ldap"
myKRB5DIR="/var/lib/kerberos/krb5kdc"
###DAEMON NOMENCLATURE
myDNSSVCNAME="named"
myDNSPACKNAME="bind"
...
sed -i 's/ExecStart=\/usr\/sbin\/named.init start/ExecStart=\/usr\/sbin\/named.init start -f -4 -u named/g' /usr/lib/systemd/system/named.service
systemctl daemon-reload
/usr/bin/chown named. /etc/named.conf
/usr/bin/chown named. $myDNSDIR/*lan
/usr/bin/chown named. $myDNSDIR/*db
rm /etc/hosts
cat >> /etc/hosts << EOF
127.0.0.1  localhost
EOF
...
zypper -n in  openldap2 openldap2-client
...
cat >> $myLDAPDATADIR/DB_CONFIG.example << EOF
set_cachesize 0 268435456 1
set_lg_regionmax 262144
set_lg_bsize 2097152
EOF

cp $myLDAPDATADIR/DB_CONFIG.example $myLDAPCONFDIR/DB_CONFIG > /dev/null 2>&1 && chown ldap. $myLDAPCONFDIR/DB_CONFIG
...
sed -i 's/OPENLDAP_CONFIG_BACKEND="files"/OPENLDAP_CONFIG_BACKEND="ldap"/g' /etc/sysconfig/openldap
sed -i 's/OPENLDAP_START_LDAPS="no"/OPENLDAP_START_LDAPS="yes"/g' /etc/sysconfig/openldap
...
cp /etc/ssl/ca-bundle.pem $myLDAPCONFDIR/ssl/
...
olcTLSCACertificateFile: $myLDAPCONFDIR/ssl/ca-bundle.pem
-
...
zypper -n in  krb5-server krb5-client
...
[libdefaults]
	...
	forwardable = true
	rdns = false
	pkinit_anchors = /etc/ssl/ca-bundle.pem
...
EOF
rm /var/lib/kerberos/krb5kdc/kdc.conf > /dev/null 2>&1
...
zypper -n in  nfs-kernel-server nfs-client
groupadd -g 65534 nobody
useradd  -r -u 65534 -g nobody -M -s /sbin/nologin nobody
[ ! -d /srv ] && mkdir /srv
...
zypper -n in  samba samba-client cifs-utils
